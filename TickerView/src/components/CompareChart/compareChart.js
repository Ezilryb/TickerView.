...
const C = {
  bg:      () => cssVar('--void', th('#040405', '#F4F1EA')),
  bgCard:  () => cssVar('--slate-elevated', th('#1A1A24', '#FFFFFF')),
  border:  () => cssVar('--border-subtle', th('#252530', '#DDD8CE')),
  textSec: () => cssVar('--text-secondary', th('#6B6872', '#6B6872')),
  magma:   () => cssVar('--magma', '#FF4D12'),
  signal:  () => cssVar('--signal', '#00E5C4'),
  magmaRaw: () => '255,77,18',
  signalRaw: () => '0,229,196',
};
  ...
  export class CompareChart {
    ...
    draw() {
      ...
      this._drawArea(ctx, L, this.normB, C.signal(), C.signalRaw(), px, py);
      this._drawArea(ctx, L, this.normA, C.magma(),  C.magmaRaw(),  px, py);
    ...
    _drawLegend(ctx, L) {
      ...
      if (this.normA.length) {
        ...
        items.push({ sym: this.symA, pct: last, color: C.magma(), rawColor: C.magmaRaw() });
      }
      if (this.normB.length) {
        const last = this.normB[this.normB.length - 1].pct;
        items.push({ sym: this.symB, pct: last, color: C.signal(), rawColor: C.signalRaw() });
      }
    ...
    _drawOverlay() {
      ...
      // Vertical crosshair
      ctx.strokeStyle = `rgba(${C.magmaRaw()},0.3)`; ctx.lineWidth = 0.8;
      ctx.setLineDash([4, 4]);
      ctx.beginPath();
      ...
      let rowY = ttY + 16;
      const entries = [];
      if (hasA) entries.push({ sym: this.symA, pct: dA.pct, color: C.magma() });
      if (hasB) entries.push({ sym: this.symB, pct: dB.pct, color: C.signal() });
