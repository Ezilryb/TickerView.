...
export function initProductPreview(options = {}) {
  ...
  // ─────────────────────────────────────────────────────────────
  function resize() {
    const wrap = canvas.parentElement; // .preview-canvas-wrap
    // getBoundingClientRect() = dimensions réelles post-layout,
    // jamais 0 même avant le premier paint complet.
    const rect = wrap.getBoundingClientRect();
    const W = Math.max(Math.round(rect.width),  1);
    const H = Math.max(Math.round(rect.height), 1);

    // Buffer pixels uniquement — pas de style.width / style.height
    canvas.width  = W * DPR;
    canvas.height = H * DPR;
    drawTab(currentTab);
  }
  ...
  // ResizeObserver uniquement sur le wrapper canvas.
  // Comme resize() n'écrit plus style.height, il n'y a aucune
  // boucle possible (modifier le buffer ne change pas le layout).
  const ro = new ResizeObserver(entries => {
    // On utilise les dimensions fournies par l'observer pour
    // éviter un second layout read (getBoundingClientRect inside)
...
function drawFootprint(ctx, W, H) {
  ...
  /* ── ✅ FIX BUG 3 : Légende ASK / BID
     Ancienne position : startX + 4 et startX + 44 → chevauchement avec les valeurs delta
     Ancienne couleur  : ASK = accent (orange), BID = signal (teal) → inversé/incohérent
     Nouvelle position : marge gauche (avant startX), en deux lignes distinctes
     Nouvelle couleur  : ASK = bleu (correspond aux valeurs ask dans les cellules)
                         BID = orange/accent (correspond aux valeurs bid dans les cellules)
  ── */
  ctx.font         = `7px 'IBM Plex Mono',monospace`;
  ctx.textAlign    = 'right';
  ctx.globalAlpha  = 0.75;

  ctx.fillStyle = bl;                               // bleu ← correspond aux ASK dans les cellules
  ctx.fillText('ASK', startX - 6, H - 14);

  ctx.fillStyle = acc;                              // orange ← correspond aux BID dans les cellules
