/* ══════════════════════════════════════════════════════════════
   ...
   FIX v2 : résolution du bug "dépli" au chargement.
   Le ResizeObserver (spec W3C) émet TOUJOURS une notification
   synchrone au moment de observe(). Cette première notification
   arrive après le premier paint et provoque un reflow qui
   "déplie" momentanément le panneau. On l'ignore avec le flag
   `skipFirst`, qui est remis à false après le premier callback.
══════════════════════════════════════════════════════════════ */
...
export function initProductPreview(options = {}) {
  ...
  function syncBuffer(W, H) {
    // Ne rien faire si les dimensions n'ont pas changé
    if (Math.round(canvas.width  / DPR) === W &&
        Math.round(canvas.height / DPR) === H) return false;
    canvas.width  = W * DPR;
    canvas.height = H * DPR;
    return true;
  }
  ...
  // ── ResizeObserver ──────────────────────────────────────────
  // FIX PRINCIPAL : le ResizeObserver (spec W3C) émet SYSTÉMATIQUEMENT
  // une notification au moment de observe(), même si la taille n'a pas
  // changé. Cette notification arrive après le premier paint et provoque
  // un reflow qui "déplie" le panneau pendant ~1 frame.
  //
  // Solution : ignorer la PREMIÈRE notification via `skipFirst = true`.
  // On remet le flag à false à l'intérieur du callback (pas dans un rAF)
  // pour que les vrais redimensionnements ultérieurs (sidebar, fenêtre)
  // soient bien traités.
  let skipFirst = true;

  const ro = new ResizeObserver(entries => {
    // Ignorer la notification initiale (émise par observe() lui-même)
    if (skipFirst) {
      skipFirst = false;
      return;
    }
    ...
    // Redessiner seulement si les dimensions ont réellement changé
    if (syncBuffer(W, H)) {
      drawTab(currentTab);
    }
  });
