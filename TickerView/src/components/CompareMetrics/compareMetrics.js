...
function fmtPrice(v) {
        ...
        <tbody>
          ${rows.map(row => `
            <tr class="cm-row">
              ...
              <td class="cm-val data-value ${row.win === 'a' ? 'cm-win-a' : ''}">${row.a}</td>
              <td class="cm-val data-value ${row.win === 'b' ? 'cm-win-b' : ''}">${row.b}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
  ...
  export function renderSymCard(elId, data, side) {
    ...
    const color  = side === 'a' ? 'var(--magma)' : 'var(--signal)';
  
    el.innerHTML = `
      <div class="csc-sym" style="color:${color}">${data.sym}</div>
      <div class="csc-name">${data.name}</div>
      <div class="csc-price data-value">${fmtP}</div>
      <div class="csc-badge ${isUp ? 'up' : 'dn'}">
        ${isUp ? '+' : ''}${data.changePct.toFixed(2)}%
      </div>
      <div class="csc-facts">
        <div class="csc-fact"><span>Exchange</span><span class="data-value">${data.exchange}</span></div>
        <div class="csc-fact"><span>Market Cap</span><span class="data-value">${data.marketCap}</span></div>
        <div class="csc-fact"><span>Volume</span><span class="data-value">${data.volume}</span></div>
